...
function _lfBuildMapperModal() {
  ...
  div.innerHTML = `
    <div class="modal-box w-full max-w-4xl max-h-[92vh] overflow-y-auto">
      ...
      <div class="p-4">
        <div id="lfMapStatus" style="font-size:.82rem;color:#E7E9EE;margin-bottom:.5rem;padding:.45rem .6rem;background:rgba(255,255,255,.08);border-radius:.4rem;border:1px solid rgba(255,255,255,.3)">اختر حقلاً ثم انقر على النموذج، أو استخدم الأسهم للضبط الدقيق (Shift = 25px, Ctrl = 1px)</div>
...
function _lfRedrawMapperCanvas() {
  ...
  Object.entries(_lfMapCoords).forEach(([f, pos]) => {
    ...
    ctx.fillStyle = (f === _lfMapField) ? 'rgba(239,68,68,.30)' : 'rgba(255,255,255,.22)';
